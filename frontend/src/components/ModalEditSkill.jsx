...
const ModalEditSkill = ({skill, onCancel}) => {
    ...
    return <div className="flex justify-center items-center bg-black/20 h-screen w-screen fixed insert-0">
        <form onSubmit={handleEditSkill} className="flex flex-col gap-4 bg-white p-8 rounded-md">
            <div>
            ...
            <div>
                ...
                <input onChange={(e) => setName(e.target.value)} type="text" className="w-full h-10 rounded-md bg-gray-200 text-gray-700 p-2" placeholder={skill.name}/>
            </div>

            <div>
                <label htmlFor="" className="text-sm text-black font-semibold">Categoria</label><br />
                <select value={category} onChange={(e) => setCategory(e.target.value)} name="" id="" className="w-full h-10 rounded-md bg-gray-200 text-gray-700 p-2">
                    {categories.map((c) =>{
            ...
            <div>
                ...
                <textarea onChange={(e) => setDescription(e.target.value)} type="text" className="w-full h-15 rounded-md bg-gray-200 text-gray-700 p-2" placeholder={skill.description}/>
            </div>
